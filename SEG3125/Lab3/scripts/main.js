...
function populateListProductChoices(slct2) {
	...
	if (document.getElementById('r2').checked) {
		optionArray = restrictListProducts(optionArray, "Lactose");
	}
	if (document.getElementById('r4').checked) {
		optionArray = restrictListProducts(optionArray, "Nuts");
	}
	...
	// Sort the array
	optionArray = optionArray.sort((x,y) => x.price - y.price);
	...
	for (i = 0; i < optionArray.length; i++) {
		...
		var imageLocation = optionArray[i].location;
...
function selectedItems(){
	...
	bold.innerHTML = getTotalPrice(chosenProducts);

	// add paragraph and total price
	c.appendChild(para);
	c.appendChild(document.createElement("br"));
	c.appendChild(document.createTextNode("Total Price: $"));
	c.appendChild(bold);
